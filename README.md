# ☕ Java With Ayush

A collection of **Java programming exercises and fundamental implementations** created while learning Core Java, problem solving, arrays, algorithms, and object-oriented programming concepts.

## 📌 About

This repository is part of my journey toward becoming a stronger software developer through consistent programming practice.

It contains small Java programs covering:

* Java fundamentals
* Arrays
* Sorting algorithms
* Searching/problem solving
* Mathematical programs
* Input and output
* Strings
* Basic programming exercises
* Logic-building problems

## 📚 Topics Covered

### 🔢 Fundamentals

Examples include:

* Input/Output
* Variables
* Operators
* Conditional statements
* Loops
* Methods
* Basic calculations

### 📦 Arrays

Practice includes:

* Array creation
* Array traversal
* Array operations
* Finding largest/second-largest values
* Basic array manipulation

### 🔃 Sorting

Implementations include:

* Bubble Sort
* Selection Sort
* Insertion Sort

These programs are useful for understanding how fundamental sorting algorithms work internally.

### 🧠 Problem Solving

The repository also contains exercises involving:

* Factorials
* Reverse operations
* FizzBuzz
* Temperature conversion
* Interest calculations
* Quadratic equations
* Random values
* Number tables

## 🛠️ Requirements

* Java JDK 8 or newer
* Any Java-compatible IDE or code editor
* Terminal/Command Prompt

## 🚀 Getting Started

Clone the repository:

```bash
git clone https://github.com/Ayush-sharma7/Java-with-Ayush.git
cd Java-with-Ayush
```

Compile a program:

```bash
javac BubbleSort.java
```

Run it:

```bash
java BubbleSort
```

Replace `BubbleSort` with the name of the Java class you want to execute.

## 📂 Repository Structure

```text
Java-with-Ayush/
├── Fundamentals
├── Arrays
├── Sorting
├── Mathematical Programs
├── Input & Output
└── Problem Solving
```

Each `.java` file is designed to demonstrate a particular programming concept or problem.

## 🎯 Purpose

The primary purpose of this repository is **learning through implementation**.

By writing programs from scratch, I am working toward:

* Better Java fundamentals
* Stronger logical thinking
* Understanding algorithms
* DSA preparation
* Better programming habits
* Building a foundation for larger Java applications

## 🌱 Future Learning

Next areas I'm working toward include:

* Object-Oriented Programming
* Collections Framework
* Exception Handling
* File I/O
* Data Structures
* Algorithms
* Competitive Programming
* Backend development with Java

## 👨‍💻 Author

**Ayush Sharma**

B.Tech CSE Student | Aspiring Software Developer

---

⭐ Explore the repository if you're learning Java and want simple examples of fundamental programming concepts.
